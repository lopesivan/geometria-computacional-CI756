...
def insertSegmentVertical(node, s):
    ...
    if intervalContainedX(node.key, s):
        node.segments = insert_y(node.segments, s.upper, False)
        return
    else:
        if node.left:
...
def insertSegmentHorizontal(node, s):
    ...
    if intervalContainedY(node.key, s):
        node.segments = insert_x(node.segments, s.upper, False)
        return
    else:
        if node.left:
...
def avlTree(P):
    ...
    height_change = False
    for point in P:
        root = insert_y( root, point, height_change )
    return root
...
# Saída: uma arvore binária balanceada
#---------------------------------------------------#
def insert_y(node, point, height_change):
    if not node:
        height_change = True
        ...
        if node.key < point.y:
            node.right = insert_y( node.right, point, height_change )
            if height_change:
                node.bal -= 1
                if node.bal == -2:
                    node = balance(node)
            ...
            node.left = insert_y( node.left, point, height_change )
            if height_change:
                node.bal += 1
                if node.bal == 2:
                    node = balance(node)
...
def balance(node):
    if node.bal < -1:
        if node.right.bal == 0:
            ...
            node.right.bal = get_balance(node.right)
        else:
            if node.right == -1:
                node = rot_left(node)
                node.bal = get_balance(node)
                ...
                if neto.bal == 0:
                    ...
                    if neto.bal > 0:
                        ...
                        node.bal = get_balance(node)
                        node.right.bal = get_balance(node.right)
                node.right.left.bal = get_balance(node.right.left)
    if node.bal > 1:
        if node.left.bal == 0:
            node = rot_right(node)
            node.bal  = get_balance(node)
            node.left.bal = get_balance(node.left)
        else:
            if node.left.bal == 1:
                node = rot_right(node)
                node.bal = get_balance(node)
                ...
                if neto.bal == 0:
                    ...
                    if neto.bal > 0:
                        ...
                        node.bal = get_balance(node)
                        node.left.bal = get_balance(node.left)
                node.left.right.bal = get_balance(node.left.right)
    return node
...
#    if balance > 1 and point.y > node.left.key:
#        print node
#        if node.key == 4 and node.height == 11:
#            print 'aooo'
#            time.sleep(2)
...
# Saída: uma arvore binária balanceada
#---------------------------------------------------#
def insert_x(node, point, height_change):
    if not node:
        height_change = True
        n = Node( point.y, point )
        return n
...
def imprime_arv(node):
    ...
    for i in xrange(node.height):
        esp = esp + "    "
    print esp, node, get_balance(node), node.bal
